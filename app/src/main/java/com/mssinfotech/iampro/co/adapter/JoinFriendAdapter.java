...
public class JoinFriendAdapter extends RecyclerView.Adapter<JoinFriendAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(MyViewHolder holder,  int positions) {
        ...
        final int[] user_block = {notifyList.get(position).getIs_block()};
        if (user_block[0] ==1){
            Glide.with(context)
        ...
        Vholder.imageView_frequest.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(FrindStatus.equals("NO")){
                    ...
                    Vholder.tv_fRequest.setText("Add To Friend");
                    FrindStatus = "NO";
                    int cntFriend = Integer.parseInt(PrefManager.getLoginDetail(context,"total_count_friend"))-1;
                    PrefManager.updateLoginDetail(context,"total_count_friend",(cntFriend)+"");
                    Config.user_text.setText(cntFriend+"");
            ...
            public void onClick(View v) {
                if (PrefManager.isLogin(context)) {
                    ...
                    yes.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            String url=Config.API_URL+ "app_service.php?type=get_block_user_detail&uid="+ PrefManager.getLoginDetail(context,"id")+"&fid="+notifyList.get(position).getId();
                            ...
                            //Toast.makeText(context,""+"Updated - "+ user_block[0],Toast.LENGTH_LONG).show();
                            myDialog.dismiss();
                        }
                    });
